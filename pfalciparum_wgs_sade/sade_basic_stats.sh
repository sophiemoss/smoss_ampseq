...
ls *.bqsr.bamstats| xargs -i -P1 sh -c 'head -7 {} | tail -1 | awk -F '"'"' '"'"' '"'"'{print $1}'"'"'' > mapped_reads
...
ls *.bqsr.bam | parallel --keep-order -j 20 --bar 'samtools depth -aa {} | awk '\''{sum+=$3} END { print sum/NR}'\''' > average_read_depth
