...
def main(args):
    ...
    samples = [l.strip() for l in open(args.samples_file)]
    # reader = csv.DictReader(open(args.samples_file))
    # if "sample" not in reader.fieldnames:
    #     reader = csv.DictReader(open(args.samples_file,encoding='utf-8-sig'))
    # for row in reader:
    ...
    if not args.per_sample_only:
        ...
        for sample in samples:
            ...
            run_cmd("cat %(sample)s.gatk.vcf | bcftools view -Oz -o %(sample)s.vcf.gz" % vars(args))
...
# If snpeff is not working in environment then it should still run to here, then inspect VCFs
# Can then take combined.vcf.gz forward using below or separate script for snpeff annotation

        ## using snpeff instead of bcftools csq
        ...
        run_cmd("bgzip snps.vcf" % vars(args))
        run_cmd("snpEff Anopheles_gambiae snps.vcf.gz > snps.ann.vcf" % vars(args))
        run_cmd("bgzip snps.ann.vcf" % vars(args))
        run_cmd("tabix snps.vcf.gz" % vars(args))
        run_cmd("tabix snps.ann.vcf.gz" % vars(args))
        run_cmd("bcftools view -v indels tmp.vcf.gz > indels.vcf" % vars(args))
        run_cmd("bgzip indels.vcf" % vars(args))
        run_cmd("snpEff Anopheles_gambiae indels.vcf.gz > indels.ann.vcf" % vars(args))
        run_cmd("bgzip indels.ann.vcf" % vars(args))
        run_cmd("tabix indels.vcf.gz" % vars(args))
        run_cmd("tabix indels.ann.vcf.gz" % vars(args))
...
parser.add_argument('--samples-file',type=str,help='Single column file with sample names',required=True)
