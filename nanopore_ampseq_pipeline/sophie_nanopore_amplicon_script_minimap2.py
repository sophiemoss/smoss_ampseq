...
# --min-base-qual 30 --threads 40 > amplicon_log.txt 2>&1
...
def main(args):
    ...
    for sample in samples:
        ...
        run_cmd("minimap2 --MD -t %(threads)s -R '@RG\\tID:%(sample)s\\tSM:%(sample)s\\tPL:nanopore' -a %(ref)s %(sample)s.fastq.gz | samclip --ref %(ref)s --max 50 | samtools sort -@ %(threads)s -o %(sample)s.bam -" % vars(args))
    ...
    with open("bam_list.txt","w") as O:
        ...
        log(f"Starting variant calling with FreeBayes for sample: {sample}")

        run_cmd("freebayes -f %(ref)s -t %(bed)s -L bam_list.txt --haplotype-length -1 "
                "--min-coverage 50 --min-base-quality %(min_base_qual)s "
                "--gvcf --gvcf-dont-use-chunk true > combined.genotyped.vcf" % vars(args))
        
        log(f"Finished variant calling with FreeBayes for sample: {sample}")
        
        run_cmd("bcftools view --threads %(threads)s -T %(bed)s combined.genotyped.vcf | "
                "bcftools norm --threads %(threads)s -f %(ref)s | bcftools sort --threads %(threads)s -Oz -o combined.genotyped.vcf.gz" % vars(args))
        
        log(f"Starting filtering for FMT/DP>10 for sample: {sample}")

        run_cmd("bcftools filter -i 'FMT/DP>10' -S . combined.genotyped.vcf.gz | "
                "bcftools view --threads %(threads)s -i 'QUAL>30' | bcftools sort --threads %(threads)s | bcftools norm -m - -Oz -o tmp.vcf.gz" % vars(args))
        
        log(f"Finished filtering for FMT/DP>10 for sample: {sample}")

        run_cmd("bcftools view --threads %(threads)s -v snps tmp.vcf.gz | bcftools csq -p a -f %(ref)s -g %(gff)s -Oz -o snps.vcf.gz" % vars(args))

        run_cmd("bcftools query snps.vcf.gz -f "
        "'[%SAMPLE\t%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t%GT\t%TGT\t%DP\t%AD\t%TBCSQ\n]' > combined_genotyped_filtered_formatted.snps.trans.txt")    
