...
def main(args):
    ...
    run_cmd("bcftools filter -i 'FMT/DP>10' -S . combined.genotyped.vcf.gz | "
            "bcftools view --threads %(threads)s -i 'QUAL>30' | bcftools sort | bcftools norm -m - -Oz -o combined.genotyped_filtered_FMTDP>10.vcf.gz" % vars(args))
    log(f"Finished filtering for FMT/DP>10")

    # SNP ANNOTATION + EXPORT
    run_cmd("bcftools view --threads %(threads)s -v snps combined.genotyped_filtered_FMTDP>10.vcf.gz | bcftools csq -p a -f %(ref)s -g %(gff)s -Oz -o snps.vcf.gz" % vars(args))

    run_cmd("(echo 'SAMPLE\tCHROM\tPOS\tREF\tALT\tQUAL\tGT\tDP\tAD\tBCSQ' ; bcftools query -f '[%SAMPLE\t%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t%GT\t%DP\t%AD\t%BCSQ\n]' snps.vcf.gz) > combined.genotyped_filtered_FMTDP>10_formatted.snps.trans.txt")

    # INDEL ANNOTATION + EXPORT
    run_cmd("bcftools view --threads %(threads)s -v indels combined.genotyped_filtered_FMT/DP>10.vcf.gz | bcftools csq -p a -f %(ref)s -g %(gff)s -Oz -o indels.vcf.gz" % vars(args))

    run_cmd("(echo 'SAMPLE\tCHROM\tPOS\tREF\tALT\tQUAL\tGT\tDP\tAD\tBCSQ' ; bcftools query -f '[%SAMPLE\t%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t%GT\t%DP\t%AD\t%BCSQ\n]' indels.vcf.gz) > combined.genotyped_filtered_FMTDP>10_formatted.indels.trans.txt")
